...
# or:
#   Rscript scripts/run_mediation.R /path/to/032_cis_mediation_fdr20
#
# Inputs (in data/): cis_eqtl_winter_bark_xylem_gene_filter_pf_all_new_fdr_20.txt,
#                    trans_eqtl_results_winter_bark_xylem_gene_filter_pf_all_new_fdr_20_atleast50genes.txt,
#                    winter_expression_matrix.csv,
#                    genotype_matrix_output1.txt
# Output: results/mediation_results.tsv

args <- commandArgs(trailingOnly = TRUE)

base_dir <- if (length(args) >= 1) args[[1]] else "."

cis_file <- file.path(base_dir, "data", "cis_eqtl_winter_bark_xylem_gene_filter_pf_all_new_fdr_20.txt")
trans_file <- file.path(base_dir, "data", "trans_eqtl_results_winter_bark_xylem_gene_filter_pf_all_new_fdr_20_atleast50genes.txt")
expr_file <- file.path(base_dir, "data", "winter_expression_matrix.csv")
geno_file <- file.path(base_dir, "data", "genotype_matrix_output1.txt")
out_file <- file.path(base_dir, "results", "winter", "mediation_results.tsv")

message("Loading cis and trans eQTL results...")
...
message("Determining overlapping SNPs for cis and trans...")
...
message("Loading expression matrix (this may take a while)...")
...
message("Preparing SNP index lookup...")
...
message("Constructing mediation triplets (all cis x trans pairs per SNP, no FDR filter)...")
setkey(cis_dt, snps)
...
if (nrow(triplets_dt) == 0L) {
  stop("No cis-trans combinations found for overlapping SNPs.")
}

message("Starting mediation regressions for ", nrow(triplets_dt), " triplets...")
...
for (i in seq_len(nrow(triplets_dt))) {
  if (i %% 1000L == 0L) {
    message("Processed ", i, " / ", nrow(triplets_dt), " triplets...")
  }
...
message("Writing mediation results to: ", out_file)
