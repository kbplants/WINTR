...
# or:
#   Rscript scripts/run_mediation_20kb_summer.R /path/to/032_cis_mediation_fdr20
#
# Inputs:
#   - results/summer/cis_trans_triplets_all_20kb_summer.tsv  (SNP–window-gene–trans triplets)
#   - data/summ_raw_cnt_mtx/gene_count_matrix_summer.csv  (summer expression matrix)
#   - data/genotype_matrix_output1.txt
# Output:
#   - results/summer/mediation_results_20kb.tsv
#
# This script reuses the same mediation regression logic as run_mediation.R,
# but operates on pre-built 20 kb window triplets for summer instead of cis/trans eQTL tables.

args <- commandArgs(trailingOnly = TRUE)

base_dir <- if (length(args) >= 1) args[[1]] else "."

triplet_file <- file.path(base_dir, "results", "summer", "cis_trans_triplets_all_20kb_summer.tsv")
expr_file <- file.path(base_dir, "data", "summ_raw_cnt_mtx", "gene_count_matrix_summer.csv")
geno_file <- file.path(base_dir, "data", "genotype_matrix_output1.txt")
out_file <- file.path(base_dir, "results", "summer", "mediation_results_20kb.tsv")

message("Loading 20 kb window SNP–window-gene–trans triplets (summer)...")
...
if (length(overlap_snps) == 0L) {
  stop("No SNPs found in 20 kb triplets file (summer).")
}
...
message("Preparing SNP index lookup (summer 20 kb)...")
...
message("Starting mediation regressions for ", nrow(triplets_dt), " summer 20 kb triplets...")
...
for (i in seq_len(nrow(triplets_dt))) {
  if (i %% 1000L == 0L) {
    message("Processed ", i, " / ", nrow(triplets_dt), " summer 20 kb triplets...")
  }
...
message("Writing summer 20 kb mediation results to: ", out_file)
dir.create(dirname(out_file), showWarnings = FALSE, recursive = TRUE)
fwrite(mediation_dt, out_file, sep = "\t")

message("Done (summer 20 kb mediation).")
