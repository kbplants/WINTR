#!/usr/bin/env Rscript
# =============================================================================
# Script: gene_filtering.R
# Purpose: Gene filtering pipeline for gene count matrix
...
results_dir <- "/mnt/winterprojectceph/new_winter/results"
output_dir <- file.path(results_dir, "gf_count_matrix")

# Input file
input_file <- file.path(results_dir, "raw_count_matrix/gene_count_matrix_winter.csv")

# Gene stats file (optional - for min expression filtering)
gene_stats_file <- "~/Downloads/gene_stats_wint.xlsx"

# Output files
output_filtered <- file.path(output_dir, "gene_count_matrix_filtered.csv")
output_vsd <- file.path(output_dir, "gene_count_matrix_vsd.csv")
output_summary <- file.path(output_dir, "gene_filtering_summary.txt")
...
cat("Gene Filtering Pipeline for Gene Count Matrix\n", file = summary_file)
cat(paste(rep("=", 60), collapse = ""), "\n\n", file = summary_file)
cat(paste(rep("=", 60), collapse = ""), "\n")
cat("Gene Filtering Pipeline for Gene Count Matrix\n")
cat(paste(rep("=", 60), collapse = ""), "\n\n")
